...
const CurriculumVitaeVisualMySelf = () => {
  ...
  useEffect(() => {
    ...
    if (userId && contactId && token) {
      axios
        .get(
          `http://localhost:8000/curriculumvitae/contactinfo/${contactId}/`,
          {
            headers: {
  ...
  useEffect(() => {
    ...
    if (userId && curriculumId && token) {
      axios
        .get(
          `http://localhost:8000/curriculumvitae/curriculumvitae/${curriculumId}/`,
          {
            headers: {
              Authorization: `Token ${token}`,
            },
      ...
      axios
        ...
        .then((response) => {
          ...
          const filteredResults = response.data.filter(
            ({ contact_info }) => contact_info == contactId
          );
  ...
  useEffect(() => {
    ...
    if (curriculumId && token) {
      axios
        .get(`http://localhost:8000/curriculumvitae/workexperience/`, {
          headers: {
            Authorization: `Token ${token}`,
          },
        })
        .then((response) => {
          // Filtrar los resultados por contactId
          const filteredResults = response.data.filter(
            (item) => item.curriculum === curriculumId
          );
  ...
  useEffect(() => {
    ...
    if (curriculumId && token) {
      axios
        .get(`http://localhost:8000/curriculumvitae/education/`, {
          headers: {
            Authorization: `Token ${token}`,
          },
        })
  ...
  useEffect(() => {
    ...
    if (curriculumId && token) {
      axios
        .get(`http://localhost:8000/curriculumvitae/skills/`, {
          headers: {
            Authorization: `Token ${token}`,
          },
        })
  ...
  useEffect(() => {
    ...
    if (curriculumId && token) {
      axios
        .get(`http://localhost:8000/curriculumvitae/interests/`, {
          headers: {
            Authorization: `Token ${token}`,
          },
        })
